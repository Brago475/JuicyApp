...
public class DivisionActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button buttonDelete = findViewById(R.id.delete2);
        ...
        buttonBack.setOnClickListener(v -> {
            Intent intent = new Intent(DivisionActivity.this, SecondActivity.class);
    ...
    private void generateNewOperation() {
        ...
        int multiplier = (int) (Math.random() * 9) + 1;
    ...
    private void showImageFeedback(String feedbackType) {
        ImageView feedbackImage = findViewById(R.id.feedback_image2);
...
public class DivisionActivity extends AppCompatActivity {
    ...
    private TextView display;
    private TextView operationTextView;
    private int firstNumber; // Dividendo
    private int secondNumber; // Divisor
    private int correctAnswer; // Respuesta correcta
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Genera una nueva operación de división
        generateNewOperation();
        ...
        // Configura el listener para el botón de verificación de respuesta
    ...
    private void generateNewOperation() {
        // Elegir un divisor aleatorio entre 1 y 9
        secondNumber = (int) (Math.random() * 9) + 1;

        // Generar un múltiplo del divisor dentro del rango de la tabla de multiplicar
        // Ejemplo: Si el divisor es 3, podemos generar: 3, 6, 9, ..., 27 (multiplicación con 1, 2, ..., 9)
        int multiplier = (int) (Math.random() * 9) + 1; // El número entre 1 y 9 con el que multiplicamos el divisor
        firstNumber = secondNumber * multiplier;

        // Asegurarse de que el primer número (dividendo) esté dentro de los límites de la tabla de multiplicar
        // Es decir, que el resultado de la multiplicación no sobrepase el rango de la tabla de multiplicar
        ...
        // Actualizamos el TextView con la operación de división
        operationTextView.setText(firstNumber + " ÷ " + secondNumber);
    }
    ...
    private void appendToDisplay(String number) {
        ...
        // Si el display contiene "=", se reinicia para ingresar un nuevo número
        if (currentText.contains("=")) {
            display.setText(number);
        } else {
            // Sino, simplemente agrega el número al display
            display.setText(currentText + number);
        }
    }

    private void checkAnswer() {
        // Extrae el texto del display y elimina cualquier carácter que no sea un número
        String userAnswerString = display.getText().toString().replaceAll("[^0-9]", "");
        int userAnswer;
        ...
        try {
            ...
            userAnswer = Integer.parseInt(userAnswerString);
            // Compara la respuesta ingresada con la respuesta correcta
            if (userAnswer == correctAnswer) {
                showToast("Good Job!");
                generateNewOperation();  // Genera una nueva operación de división
                display.setText("");  // Limpia el display para la siguiente operación
            } else {
                showToast("Try Again!");
                display.setText("");  // Limpia el display para intentar otra vez
            }
        } catch (NumberFormatException e) {
            showToast("Please enter a number!");  // Si no se ingresó un número válido
        }
    }
